        ...
        for line in lines:
            ...
            if not SiteGroups:
                for i,protein in enumerate(proteins):
                    ...
                    protein_sites.append([protein, gene, prositegroup, prosites[i], prosites[i], "", "", "", "", "", mono, isotope, peptide, glycan, prosites[i], mod, spec, raw, scan, ETDscan])
            else:
                for s1,s2,site_glycan,prob in SiteGroups:
                    ...
                    for i,protein in enumerate(proteins):
                        ...
                        protein_sites.append([protein, gene, prositegroup, prosite1, prosite2, site_glycan, prob, "1" if prosite1==prosite2 else "0", "1" if prosite1!=prosite2 else "0", s1+':'+s2, mono, isotope, peptide, glycan, str(prosite), mod, spec, raw, scan, ETDscan])
    protein_sites.sort(key = lambda x: (x[0], int(x[3]), int(x[4])))
    protein_sites.insert(0, ["Protein", "Gene", "ProteinSiteGroup", "StartSite", "EndSite", 'Localized'+glycan_head, "SiteProbability", "IsUniqueSite", "IsGroupSite", "PeptideSiteGroup", "MonoArea", "IsotopeArea", "Peptide", "AllSiteGlycan", "ProSite", "Mod", "Spectrum", "RawName", "Scan", "ETDScan"])
